...
/**
 * Base class for table test suites.
 *
...
public abstract class AbstractTableTestSuiteBase {
    ...
    protected String getCreateTableSql(
        ...
        String createTableSql =
                String.format(
                        "CREATE TABLE `%s` (\n" + " %s\n" + ") WITH (\n" + "%s\n" + ")",
                        tableName, getSchema(dataTypes), getTableOptions(tableOptions));
    ...
    private String getSchema(List<DataType> dataTypes) {
                ...
                .mapToObj(
                        (index) -> {
                            ...
                            return String.format(
                                    "`%s` %s",
                                    getTestFieldName(index, dataType), getTypeSql(dataType));
                        })
                .collect(joining(",\n"));
    }
                                ...
                                .mapToObj(
                                        index -> {
                                            ...
                                            return Column.physical(
                                                    getTestFieldName(index, dataType), dataType);
                                        })
    ...
    private String getTypeSql(DataType type) {
        return type.getLogicalType().toString();
    }

    private String getTestFieldName(int index, DataType type) {
